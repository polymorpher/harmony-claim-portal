...
# the batch job must recover each signer before promoting a wallet.
set -euo pipefail
...
\\copy (SELECT id, address, data_version, policy_version, stage_reason, message, signature, signer, created_at FROM confirm.confirmations ORDER BY created_at, address) TO '${out}/confirmations.csv' WITH (FORMAT csv, HEADER true)
